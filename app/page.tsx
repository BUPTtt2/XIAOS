          ...
          {script && status === 'done' && (
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-gray-800">剧本预览</h2>
                <button
              ...
              <div className="space-y-6">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-sm">标题: {script.meta.title}</p>
                  <p className="text-sm">作者: {script.meta.author}</p>
                  <p className="text-sm">改编者: {script.meta.adaptedBy}</p>
                </div>

                <div className="bg-gray-50 rounded-lg p-4">
                  <h3 className="font-semibold text-gray-700 mb-2">出场人物</h3>
                  <ul className="space-y-1">
                    {script.characters.map((char, index) => (
                      <li key={index} className="text-sm">
                        <span className="font-medium">{char.name}</span>
                        {char.description && <span className="text-gray-500"> - {char.description}</span>}
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="space-y-4">
                  <h3 className="font-semibold text-gray-700">场景</h3>
                  {script.scenes.map((scene) => (
                    <div key={scene.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex gap-4 text-sm text-gray-500 mb-2">
                        {scene.title && <span className="font-medium text-gray-700">{scene.title}</span>}
                        {scene.time && <span>时间: {scene.time}</span>}
                        {scene.location && <span>地点: {scene.location}</span>}
                      </div>
                      {scene.description && (
                        <p className="text-sm text-gray-600 mb-3 italic">{scene.description}</p>
                      )}
                        ...
                        {scene.elements.map((element, index) => {
                          const renderElement = () => {
                            switch (element.type) {
                              case 'dialogue':
                                return (
                                  <div key={index} className="ml-4">
                                    <span className="font-medium text-purple-600">{element.character}</span>
                                    {element.tone && <span className="text-gray-400 text-xs ml-2">({element.tone})</span>}
                                    <p className="text-gray-800">{element.content}</p>
                                    {element.action && <p className="text-sm text-gray-500 italic">{element.action}</p>}
                                  </div>
                                );
                              case 'action':
                                return (
                                  <p key={index} className="text-sm text-gray-700">
                                    {element.character && <span className="font-medium">{element.character} </span>}
                                    <span className="italic">{element.content}</span>
                                  </p>
                                );
                              case 'narration':
                                return (
                              ...
                              case 'transition':
                                return (
                                  <p key={index} className="text-sm text-blue-500 font-medium text-center py-2">
                                    → {element.content}
                                  </p>
                                );
                              default:
